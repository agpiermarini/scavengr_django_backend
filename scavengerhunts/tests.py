...
class ScavengerHuntEndpointTestCase(TestCase):
    ...
    def test_user_specific_scavenger_hunt_index_endpoint(self):
        ScavengerHunt.objects.create(id=1, name=self.name, description=self.description, user_id=self.user2.id)
        ScavengerHunt.objects.create(id=2, name="second_hunt", description="second_description", user_id=self.user2.id)
        ScavengerHunt.objects.create(id=3, name="third_hunt", description="third_description", user_id=self.user.id)
        response = self.client.get(f'/api/v1/users/{self.user2.username}/scavenger_hunts/')
        ...
        self.assertEqual(response.json()[0]['name'], self.name)
        self.assertEqual(response.json()[0]['description'], self.description)
        self.assertEqual(response.json()[0]['user_id'], self.user2.id)
        self.assertEqual(response.json()[0]['username'], self.user2.username)
        self.assertEqual(response.json()[1]['id'], 2)
        self.assertEqual(response.json()[1]['name'], "second_hunt")
        self.assertEqual(response.json()[1]['description'], "second_description")
        self.assertEqual(response.json()[1]['user_id'], self.user2.id)
        self.assertEqual(response.json()[1]['username'], self.user2.username)
...
class CurrentScavengerHuntEndpointTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.scavenger_hunt = ScavengerHunt.objects.create(name="scavenger hunt", description="tour denver's best speakeasies")
